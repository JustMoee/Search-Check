...
const generateRandomData = () => {
  ...
  const IDAccount = [1001, 1002, 1003, 1004, 1005];
  ...
  const randomIndexID = Math.floor(Math.random() * IDAccount.length);
  ...
  const sql = `INSERT INTO compData (nameEN, nameAR, nameC, expiryDate, ContractStatus, accID, services, notes) 
               VALUES (?, ?, ?, ?, ?, ?, ?, ?)`;
  ...
  db.run(sql, [namesEN[randomIndexEN], namesAR[randomIndexAR], codes[randomIndexC], 
               ...
               IDAccount[randomIndexID], services[randomIndexServices], notes[randomIndexNotes]], 
    (err) => {
      if (err) {
